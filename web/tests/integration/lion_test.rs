...
async fn index_page_works_for_authenticated_users(pool: DbPool) {
    authenticated_request::<_, _>(pool.clone(), |request| async move {
        let response = request.get("/lions").await;
...
#[sqlx::test(migrator = "MIGRATOR")]
async fn create_lion_redirects_and_displays_in_ui(pool: DbPool) {
    let lion: LionChangeset = Faker.fake();

    test_request_with_db::<_, _>(pool, |request| async move {
        let response = request.post("/lions").form(&lion).await;
        ...
        response.assert_text_contains(lion.name);
        response.assert_text_contains(lion.email);
    })
    .await
}
//
